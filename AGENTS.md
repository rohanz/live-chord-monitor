...
- The window is created with `show: false` and shown from `ready-to-show`. Measured from a Finder launch, the window was previously mapped ~230ms before first paint, so the app looked open while it could not yet register a note. The `ready-to-show` handler is registered BEFORE the `isDev` early-return - with `show: false` it is the only thing that ever makes the window visible, so moving it below that return would leave the dev window permanently hidden.
- Startup budget, measured from `open` on an arm64 dev machine (warm): ~1.75s of macOS/Electron process launch before any app code runs, then ~55ms to window creation, ~105ms to bundle evaluation, ~105ms to first paint, ~115ms to `requestMIDIAccess` resolving. The app-controlled portion is ~450ms of a ~2.2s total; the launch overhead dominates and is not addressable in app code. Web MIDI was measured with no device attached, so device enumeration may add to that last figure.
- An `AudioContext` constructed with NO user gesture was measured as born `running` in this Electron build (it stayed `running` after 3s), so the autoplay policy is not a source of first-note silence here. The gesture priming in `usePianoAudio` is still correct defensively - do not assume it is dead code and remove it.
- ASCII `b`/`#` are the internal representation everywhere; `toAccidentalGlyphs` (`src/utils/accidentals.ts`) rewrites them to `♭`/`♯` at RENDER TIME ONLY, at six call sites (`App.tsx` chord name + alternatives + both range labels, `PianoKeyboard.tsx` both pressed-key labels, `SettingsDrawer.tsx` spelling options). Do NOT push glyphs into `notes.ts`, `buildSpelling`, chord suffixes, or `SpellingKey`: `StaffNotation` parses a spelling with `/^([A-G])([b#]{0,2})$/` and hands the accidental straight to VexFlow, which only understands ASCII; `chords.ts` string-matches ASCII throughout (`SYMBOL_REWRITES`, `degreeForInterval`'s `suffix.includes('#9')`, `ADDITION_DEGREES` keys); and `SpellingKey` values are persisted settings. The `<option value>` attributes stay ASCII for that reason - only the option TEXT is glyphed.
